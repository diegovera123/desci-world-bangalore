...
function toggle() {
  // set the prefer color scheme
  const scheme = isLightOn.value ? "light" : "dark";

  document.documentElement.setAttribute("data-theme", scheme);
}

watch(isLightOn, toggle);

onMounted(() => {
  // set the prefer color scheme from the user's system
  isLightOn.value = window.matchMedia("(prefers-color-scheme: dark)").matches
    ? false
    : true;
});
</script>
...
<template>
  <!-- <button class="text" @click="toggle">Toggle</button>
